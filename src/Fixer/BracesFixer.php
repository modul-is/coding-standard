...
',
	...
	private function fixSpaceAroundToken(Tokens $tokens): void
	{
		...
		for($index = $tokens->count() - 1; 0 <= $index; --$index)
		{
			...
			elseif($token->isGivenKind($controlTokens))
			{
	...
	private function fixDeclareStatement(Tokens $tokens, $index): void
	{
	...
	private function fixSingleLineWhitespaceForDeclare(Tokens $tokens, $startBraceIndex): void
	{
