...
namespace ModulIS\Sniffs\Numbers;
...
class RequireNumericLiteralSeparatorSniff implements Sniff
{
	...
	/** @var int */
	public $minDigitsBeforeDecimalPoint = 7;

	/** @var int */
	public $minDigitsAfterDecimalPoint = 20;
	...
	public function process(File $phpcsFile, $numberPointer): void
	{
		...
		$fix = $phpcsFile->addFixableError('Use of numeric literal separator is required.', $numberPointer, self::CODE_REQUIRED_NUMERIC_LITERAL_SEPARATOR);
		...
		if(is_numeric($tokens[$numberPointer]['content']))
		{
			$decimalCount = (int) strpos(strrev($number), '.');

			$phpcsFile->fixer->replaceToken($numberPointer, number_format((float) $number, $decimalCount, '.', '_'));
		}
